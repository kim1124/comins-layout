...
- Desktop Chromium is the required automated compatibility target. Firefox and Playwright WebKit projects remain available for explicit, non-gating checks. Mobile touch coverage remains on the Pixel 7 Chromium profile.
...
- Branded Safari on macOS and iOS is not directly verified. The optional Playwright WebKit project is an engine-compatibility signal, not a Safari device certification.
