...
복사 모드에서는 원래 자리에 원본 모양을 남기고 + 윤곽선만 포인터를 따라갑니다. 이 정적 미리보기는 드래그 시작 시점의 표시용 복제이며 별도 React 위젯이나 엔진 노드가 아닙니다. 드롭·취소 시 제거됩니다. 같은 Grid 안에 놓으면 기존처럼 위치만 변경하며, 복사는 다른 Grid로 전송할 때 적용됩니다.

source와 target Grid의 컬럼 너비가 달라도 adapter는 포인터로 잡은 지점의 비율을 보존해 target 셀 좌표로 환산합니다. 최종 배치는 대상의 컬럼 경계와 기존 위젯 충돌에 따라 보정될 수 있습니다.

Grid source request는 `transferDashboardWidget`에 source/target 상태와 request 좌표, snapshot, widget ID, mode를 전달합니다. 승인 결과의 양쪽 상태를 `serializeDashboardState` 후 각 hook에 `restoreLayout`합니다. `move`는 source에서 위젯을 제거하고, `copy`는 source에 남깁니다. 두 모드 모두 ID를 유지하므로 target에 같은 ID가 있으면 거부됩니다. `copy`가 새 ID를 자동 생성하지는 않습니다.
...
Playground: `/examples/advanced/multi-grid/horizontal`. 페이지의 배치 방향 컨트롤로 전송 계약은 유지한 채 가로/세로 Grid 배치를 비교합니다. 키보드용 전송 버튼도 같은 helper를 사용하지만 대상 Grid의 하단에 배치합니다. 드래그는 놓은 위치를 사용합니다.
